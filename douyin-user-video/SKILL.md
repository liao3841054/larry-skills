...
**重要：** 当用户请求下载博主主页视频时，**必须先询问以下问题**，待用户确认后再执行。
...
| 请求类型 | 特征 | 是否需要询问 |
|----------|------|--------------|
| 单视频下载/提取 | 提供视频分享链接 | 否，直接执行 |
| 主页视频列表 | 提供用户主页链接 | **是，必须询问** |
| 批量下载主页视频 | "下载主页所有视频"、"获取博主视频" | **是，必须询问** |
...
1. 主页视频列表获取方式：
   - A. Playwright（免费，需要浏览器环境，当前环境可用时推荐）
   - B. 10倍猫 API（稳定，需要 API Key，适合无头环境）
...
```
1. 获取主页视频列表
   - 方式 A: python scripts/douyin_user_videos.py --url "主页链接"
   - 方式 B: python scripts/douyin_user_videos.py --url "主页链接" --method meow --api-key "xxx"

2. 读取 videos.json，获取前 N 个或全部视频

3. 逐个下载视频
   - 仅下载: --action download
   - 需要字幕: --action extract --save-video
```
...
用户提供链接
    ...
    └── 主页链接 → 询问用户
                      │
                      ├── 选择 Playwright → 检查环境 → 执行
                      │
                      └── 选择 10倍猫 API → 收集 API Key → 执行
```
